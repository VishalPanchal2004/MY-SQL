...
select  c.customer_name , count(o.order_id)as total_order from   customers c join orders o on c.customer_id = o.customer_id  group by c.customer_name ;

#Show each customer and their total purchase amount.
select  c.customer_name , sum(o.total_amount)as total_order from   customers c join orders o on c.customer_id = o.customer_id  group by c.customer_name ;

#3. Count the number of orders placed per country.
select  c.country, count(o.order_id) as total_orders  from customers c inner join  orders o on c.customer_id = o.customer_id group by c.country;

#4. Show total sales grouped by country.
select c.country , sum(o.total_amount) as total_sale from customers c inner join  orders o on c.customer_id = o.customer_id group by c.country;

#5. Find the average order value (AOV) per customer.
select c.customer_name , avg(o.order_id) as Average_total from   customers c join orders o on c.customer_id = o.customer_id  group by c.customer_name ;

#6. Show each product and total units sold.
select p.product_name , count(od.product_id) from products p join order_details od on p.product_id  = od.product_id group by p.product_name ;

#7. Find total revenue generated per product category.
select p.product_name , sum(od.unit_price) from products p join order_details od on  p.product_id  = od.product_id group by p.product_name ;
select p.product_name , sum(p.unit_price) from products p   group by p.product_name ;


#8. Group by month from order_date and show total order count.
select date_format(o.order_date , "%m") as order_month , count(o.order_id) as order_count from customers c join orders o
 on c.customer_id = o.customer_id  group by order_month ;
 
 select monthname(o.order_date ) as order_month ,
 count(o.order_id) as order_count from customers c join orders o
 on c.customer_id = o.customer_id  group by order_month ;
 
 #9. Show monthly total sales.
 select monthname(o.order_date),  sum(o.total_amount) as total_sale  from orders group by total_sale ;
 select monthname(o.order_date ) as order_month ,
 sum(o.total_amount) as order_count from customers c join orders o
 on c.customer_id = o.customer_id  group by order_month ;
 
 #10. Show each product and its average quantity sold.
 select p.product_name , avg(od.quantity) as Avg_Quantity from products p  join order_details od on p.product_id =od.product_id group by product_name ;
